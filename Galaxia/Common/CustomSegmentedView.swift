...
class SSSegmentedControl: UIView {
    ...
    @IBInspectable var currentIndexBackgroundColor: UIColor = .white {
        didSet {
        ...
        stackView.subviews.enumerated().forEach { (index, view) in
            ...
            if index == currentIndex {
                ...
                if animated {
                    UIView.animate(withDuration: 0.3) {
                        self.currentIndexView.frame =
                            CGRect(x: self.buttonPadding + (buttonWidth * CGFloat(index)),
                                   y: self.frame.height - self.buttonPadding,
                               width: buttonWidth,
                               height: 2)
                    }
                } else {
                    self.currentIndexView.frame =
                        CGRect(x: self.buttonPadding + (buttonWidth * CGFloat(index)),
                               y: self.frame.height - buttonPadding,
                           width: buttonWidth,
                           height: 2)
                }
    ...
    private func addSegments() {
        ...
        for index in 0 ..< numberOfSegments {
            ...
            button.titleLabel?.font = .fontBold(ofSize: 12)
